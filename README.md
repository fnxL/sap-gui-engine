...
table_name_field = session.find_by_id("wnd[0]/usr/ctxtDATABROWSE-TABLENAME")
table_name_field.text = "MARA"
...
```python
from sap_gui_engine import SAPGuiEngine
from sap_gui_engine.exceptions import SAPConnectionError, SAPLoginError, SAPTransactionError
...
The library provides specific exception classes for different error conditions:

- `SAPConnectionError`: Raised when connection establishment fails
- `SAPElementNotFound`: Raised when a requested GUI element is not found
- `SAPElementNotChangeable`: Raised when attempting to modify a read-only element
- `SAPComboBoxOptionNotFound`: Raised when a specified option is not available in a ComboBox
- `SAPStatusBarError`: Raised when there is an error in the status bar
- `SAPTransactionError`: Raised when a transaction fails to start or encounters a critical error
- `SAPLoginError`: Raised when the login process fails
- `SAPElementTypeMismatch`: Raised when the element type does not match the expected type
- `SAPTableConfigurationError`: Raised when there is a mismatch or error in table configuration/population
